...
///
/// Same flow as `full_lifecycle.rs` but with Token 2022 (with metadata
/// extensions) throughout. Verifies the IFT program's `TokenInterface`
...
async fn test_ift_token_2022_lifecycle() {
    ...
    let final_balance = token_kind.read_balance(&chain, user_ata).await;
