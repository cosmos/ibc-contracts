...
/// Accounts for [`require_role`](access_manager::require_role) wrapper.
#[derive(Accounts)]
...
mod integration_tests {
    ...
    fn build_ix(access_manager: Pubkey, signer: Pubkey, role_id: u64) -> Instruction {
        let accounts = crate::accounts::CheckRequireRole {
            access_manager,
            signer,
        ...
        Instruction {
            ...
            data: crate::instruction::CheckRequireRole { role_id }.data(),
        }
    }
    ...
    #[tokio::test]
    async fn test_cpi_always_rejected() {
        let relayer = Keypair::new();
        let mut pt = setup_program_test();
