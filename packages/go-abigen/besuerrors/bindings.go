...
// BindingsUnknownCommitSealSigner represents a UnknownCommitSealSigner error raised by the Bindings contract.
type BindingsUnknownCommitSealSigner struct {
	Signer common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error UnknownCommitSealSigner(address signer)
func BindingsUnknownCommitSealSignerErrorID() common.Hash {
	return common.HexToHash("0x67bb2f2e106256c281fb496588b670f4592ae8ad1fada070efe49d57c82e6354")
}

// UnpackUnknownCommitSealSignerError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error UnknownCommitSealSigner(address signer)
func (bindings *Bindings) UnpackUnknownCommitSealSignerError(raw []byte) (*BindingsUnknownCommitSealSigner, error) {
	out := new(BindingsUnknownCommitSealSigner)
	if err := bindings.abi.UnpackIntoInterface(out, "UnknownCommitSealSigner", raw); err != nil {
		return nil, err
	}
	return out, nil
}
