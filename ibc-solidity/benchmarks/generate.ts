...
export function renderReadme(
  ...
  const lines = [
    ...
    "| ---: | ---: | ---: |",
    ...BESU_QBFT_SIM_VALIDATOR_COUNTS.map(
      (count) =>
...
export function assertGeneratedContent(
  ...
  if (actual !== expected) {
    throw new Error(
      `${path} is stale; run \`just solidity::benchmark-update\` and commit the result`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  await runForge(mode);
  const [sp1, besu, besuSim] = await Promise.all([
    readSnapshot("SP1E2E.json", SP1_KEYS),
    readSnapshot("BesuQBFT.json", BESU_QBFT_KEYS),
    readSnapshot("BesuQBFTSim.json", BESU_QBFT_SIM_KEYS),
  ]);
  const expectedReadme = renderReadme(sp1, besu, besuSim);
